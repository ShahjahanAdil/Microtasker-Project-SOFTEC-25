...
export default function Notifications() {
    ...
    const fetchNotifications = (page) => {
        setLoading(true)
        axios.get(`${import.meta.env.VITE_HOST}/admin/notifications?userID=${user?.userID}&page=${page}`)
            .then(res => {
    ...
    const handleClearNotifications = () => {
        ...
        axios.delete(`${import.meta.env.VITE_HOST}/admin/notifications/clear-all?userID=${user?.userID}`)
            .then(res => {
                const { status, data } = res
                if (status === 203) {
                    setNotifications([])
