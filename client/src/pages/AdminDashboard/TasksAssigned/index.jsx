...
export default function TasksAssigned() {
    ...
    const handleDeleteTask = () => {
        setDelLoading(true)
        axios.delete(`${import.meta.env.VITE_HOST}/admin/tasks-assigned/delete?taskID=${delTaskID}`)
            .then(res => {
                const { status, data } = res
                if (status === 203) {
                    window.toastify(data.message, "success")
                    const updatedAssignedTasks = allAssignedTasks.filter(task => task.taskID !== delTaskID)
                    setAllAssignedTasks(updatedAssignedTasks)
                    setDelTaskID("")
                    setOpenModel(false)
                }
            })
    ...
    const handleUpdateTask = () => {
        ...
        axios.patch(`${import.meta.env.VITE_HOST}/admin/tasks-assigned/update`, updatingTask)
            .then(res => {
                const { status, data } = res
                if (status === 202) {
                    window.toastify(data.message, "success")
                    const updatedAssignedTasks = allAssignedTasks.map(task => task.taskID === updatingTask.taskID ? updatingTask : task)
                    setAllAssignedTasks(updatedAssignedTasks)
                    setUpdatingTask({})
