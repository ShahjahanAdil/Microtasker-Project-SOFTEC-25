...
export default function PendingTasks() {
    ...
    const handleFetchData = () => {
        setLoading(true)
        axios.get(`${import.meta.env.VITE_HOST}/user/pending-tasks?userID=${user?.userID}`)
            .then(res => {
                ...
                <div className='p-3 rounded-[5px]'>
                    <h6 className='font-bold text-[20px] text-[#333] flex items-center gap-2'>
                        <p className='flex items-center gap-2'>
                            ...
                            <span className='text-[18px] font-normal text-[#666]'>/ Pending Tasks</span>
                        </p>
                    </h6>
                </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className='bg-[#5272ffa3]'>
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3 text-start text-[13px] font-bold text-[#fff] uppercase whitespace-nowrap">Joined ON</th>
                                                                ...
                                                                return (
                                                                    <tr key={taskID} className="odd:bg-white even:bg-[#f3f6ff] hover:bg-[#f3f6ff]">
                                                                        ...
                                                                        <td className="px-6 py-4 whitespace-nowrap text-end text-[14px] text-[#666]">
                                                                            <Link to={`/user/your-tasks/${taskID}`} className='font-bold !text-[#5271ff] flex gap-2 justify-end items-center transition-all !duration-150 !hover:text-[#333] hover:underline'>
